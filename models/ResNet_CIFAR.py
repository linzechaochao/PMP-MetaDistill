...
class ResidualBlock(nn.Module):
    def __init__(self, mode, inchannel, cfg, ones, outputs, stride=1):
        ...
        if mode=='sparse':
            ...
            self.left = nn.Sequential( 
                SparseConv2d(inchannel, cfg[0], kernel_size=3, k=cfg[0]*inchannel , rho_init=0.01,  rho_maximum=6, mu=1.01, stride=stride, padding=1,bias=False),
                nn.BatchNorm2d(cfg[0]),
                nn.ReLU(inplace=True),
                SparseConv2d(cfg[0], cfg[1], kernel_size=3, k=cfg[1]*cfg[0] , rho_init=0.01,  rho_maximum=6, mu=1.01, stride=1, padding=1,bias=False),
                nn.BatchNorm2d(cfg[1])
        )
